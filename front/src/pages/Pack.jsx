...
const Pack = () => {
  ...
  const [productList, setProductList] = useState(packs);
  const [onSelectedDelete, setOnSelectedDelete] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');

  const onProductClick = (pack) => {
    {
  ...
  const onDeleteClick = (pack) => {
    setDeleteItemModal(true);
    setOnSelectedDelete(pack);
  };

  const onFinalDeleteClick = (pack) => {
    setDeleteItemModal(false);
    deleteProduct(pack._id);
  };

  const deleteProduct = (productId) => {
    setDeleteItemModal(false);
    setProductList(productList.filter((pack) => pack._id !== productId));
  };
    ...
    <Dashboard>
      ...
      <Modals open={openAdd} onClose={() => setOpenAdd(false)}>
        <div className="flex flex-col gap-2 min-w-80">
          <h1 className="text-2xl mt-2">Ajouter un pack</h1>
          <label htmlFor="nomPack">
          ...
          />
          <label htmlFor="localisationPack">
      ...
      {/* Modal for displaying a pack entry */}
      <Modals
        open={openListItem}
        onClose={() => {
      ...
      >
        {selectedProduct && (
          <div className="m-8 text-center flex gap-4 flex-col capitalize">
            ...
            <p>Localisation: {selectedProduct.localisation}</p>
          </div>
        )}
      </Modals>

      {/* Modal for deleting a pack entry */}
      <Modals
      ...
      >
        {onSelectedDelete && (
          <div className="flex flex-col gap-4 min-h-52 min-w-52 justify-center items-center">
            ...
            <p>{onSelectedDelete._id}</p>
            <p>{onSelectedDelete.nom}</p>

            <div className="flex flex-row gap-10 justify-between">
              <button
                onClick={() => onFinalDeleteClick(onSelectedDelete._id)}
      ...
      <div>
        <div className="flex flex-row justify-between py-2 bg-gray-200 dark:bg-gray-700">
          <p className='w-1/4 justify-center flex'>Nom du pack</p>
          <p className='w-1/4 justify-center flex'>Matricule</p>
          <p className='w-1/4 justify-center flex'>Localisation</p>
        ...
        <div className="flex flex-col overflow-y-scroll max-h-80 max-w-full relative pb-1 pr-1 h-96">
          {filteredProduct.map((index) => (
            <div
              ...
              key={index.id}
            >
              <p className="w-1/4 px-4 justify-center flex">{index.nom}</p>
              <p className="w-1/4 justify-center flex">{index.matricule}</p>
              <p className="w-1/4 justify-center flex">{index.localisation}</p>
