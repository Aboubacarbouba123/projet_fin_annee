...
const Gardien = () => {
  ...
  const [productList, setProductList] = useState(gardien);
  ...
  const filteredProduct = productList.filter(
    (product) =>
      product.nom_gardien &&
      product.nom_gardien.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <Dashboard>
      <Modals open={openAdd} onClose={() => setOpenAdd(false)}>
        <div className="flex flex-col gap-2 min-w-80">
          <h1 className="text-2xl mt-2">Ajouter un gardien</h1>
      ...
      >
        {selectedProduct && (
          <div className="m-8 text-center flex gap-4 flex-col capitalize">
            <h2 className="text-2xl pb-2 "> {selectedProduct.nom_gardien}</h2>
        ...
        <div className="flex flex-col overflow-y-scroll max-h-80 max-w-full relative pb-1 pr-1 h-96">
          {filteredProduct.map((index) => (
            <div
              ...
              key={index.id}
            >
              <p className="w-1/4 justify-center flex">{index.nom_gardien}</p>
