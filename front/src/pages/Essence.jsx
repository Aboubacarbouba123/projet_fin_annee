...
import {essences} from '../mocks/Mock2'
...
const Essence = () => {
  ...
  const [productList, setProductList] = useState(essences);
  ...
  const filteredProduct = productList.filter((product) =>
    product.nom_essence && product.nom_essence.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <Dashboard>
      ...
      <Modals open={openAdd} onClose={() => setOpenAdd(false)}>
        <div className="flex flex-col gap-2 min-w-80">
          <h1 className="text-2xl mt-2">Ajouter une essence</h1>
      ...
      >
        {selectedProduct && (
          <div className="m-8 text-center flex gap-4 flex-col capitalize">
            <h2 className="text-2xl pb-2 "> {selectedProduct.nom_essence}</h2>
      ...
      >
        {onSelectedDelete && (
          <div className="flex flex-col gap-4 min-h-52 min-w-52 justify-center items-center">
            ...
            <p>{onSelectedDelete.nom_essence}</p>
      ...
      <div>
        <div className="flex flex-row justify-between py-2 bg-gray-200 dark:bg-gray-700">
          <p className='w-1/4 justify-center flex'>Nom essence</p>
        ...
        <div className="flex flex-col overflow-y-scroll max-h-80 max-w-full relative pb-1 pr-1 h-96">
          {filteredProduct.map((index) => (
            <div
              ...
              key={index._id}
            >
              <p className="w-1/4 justify-center flex">{index.nom_essence}</p>
